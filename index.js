...
intents.matches('whatisWeather', [function (session, args) {
    ...
    if (city) {
        ...
        var url = "http://api.apixu.com/v1/current.json?key=01e8fb3953914ecda54135946172306&q=" + city_name;
    ...
    var city_name = results.response; var url = "http://api.apixu.com/v1/current.json?key=01e8fb3953914ecda54135946172306&q=" + city_name;
    request(url, function (error, response, body) {
        body = JSON.parse(body);
        temp = body.current.temp_c;
        session.send("It's " + temp + " degrees celsius in " + city_name);
